...
export function Todo() {
  ...
  function addNewItem(event: React.FormEvent) {
    ...
    if (isNewItemContent) return false;
  ...
  return (
    <main className={styles.todoContainer}>
      <form className={styles.todoInput}>
        ...
        />
        <button type="submit" disabled={isNewItemContent} onClick={addNewItem}>
          <span>Criar</span>
          <PlusCircle size={20} />
        </button>
      </form>
      <div className={styles.todoListContainer}>
        <div className={styles.todoStatus}>
          <div>
            ...
            <strong className={styles.statusNumber}>{todoList.length}</strong>
          </div>
          <div>
            <strong className={styles.finishedStatusText}>Concluídas</strong>
            <strong className={styles.statusNumber}>
              {todoList.length == 0
                ? 0
                : `${getCompletedAmount()}` + " de " + `${todoList.length}`}
            </strong>
          </div>
        </div>
